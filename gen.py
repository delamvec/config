...
def genMain(oSub={}, szParentName=[]):
	...
	for v1 in oSub:
		...
		if v1["type"]==M2TYPE.DB:
			...
			# Přidej P2P_ACCEPT pro ostatní cores ve stejném kanálu
			if v1["type"] == M2TYPE.CORE:
				# Získej všechny cores ve stejném kanálu
				cores_in_channel = [core for core in serverinfolist 
					if core["type"] == M2TYPE.CORE 
					and core["chan"] == kh1 
					and core["serv"] == genConfig["active"]]
				
				# Přidej P2P_ACCEPT konfigurace
				WriteP2PConfig("%s/CONFIG"%szTmpParentName, 
					{"path": szTmpParentName}, 
					cores_in_channel)
			# w/o array, only if
			wTmpPort={}
