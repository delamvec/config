...
def WriteP2PConfig(szConfFile, current_core_info, all_cores_in_channel):
	"""Generuje P2P_ACCEPT řádky pro komunikaci mezi cores ve stejném kanálu"""
	for core in all_cores_in_channel:
		# Nepřidávej sám sebe
		if core["path"] == current_core_info["path"]:
			continue
		
		# Přidej P2P_ACCEPT pro ostatní cores ve stejném kanálu
		p2p_port = core.get("p2p_port", 0)
		if p2p_port:
			Append2File(f"P2P_ACCEPT: {v_bindHostname} {p2p_port}", szConfFile)
	...
	def calcPortList():
		...
		for elem in serverinfolist:
			if elem["type"]==M2TYPE.AUTH:
				...
				if newpath not in _portlist and elem["chan"] != 99:
					_portlist[newpath] = elem["port"]
...
def genInitSrv(szSvr):
	...
	#
	for val1 in ("share/conf/BANIP","share/conf/CMD","share/conf/CRC","share/conf/VERSION","share/conf/state_user_count"):
			...
			serverinfolist.append(
				{
					...
					"p2p_port":wTmpPort[PORT.P2P_PORT],
					"type":v1["type"],
					"chan":kh1,
					"serv":genConfig["active"],
				}
...
def genGen():
	genMain()
	genPostProcessP2P()  # Přidej P2P konfigurace po dokončení generování
	genList(True)

def genPostProcessP2P():
	"""
	Post-processing: Doplní P2P_ACCEPT do všech core CONFIG souborů
	Po dokončení generování máme kompletní seznam všech cores
	"""
	# Seskup cores podle serveru a kanálu
	cores_by_channel = {}
	
	for core in serverinfolist:
		if core["type"] != M2TYPE.CORE:
			continue
		
		key = (core["serv"], core["chan"])
		if key not in cores_by_channel:
			cores_by_channel[key] = []
		cores_by_channel[key].append(core)
	
	# Pro každý kanál přidej P2P_ACCEPT do všech cores
	for channel_key, cores in cores_by_channel.items():
		serv, chan = channel_key
		print(f"Post-processing P2P for {serv} CH{chan}: {len(cores)} cores")
		
		for current_core in cores:
			config_path = f"{current_core['path']}/CONFIG"
			
			# Přidej P2P_ACCEPT pro všechny ostatní cores ve stejném kanálu
			for other_core in cores:
				if other_core['path'] == current_core['path']:
					continue  # Přeskoč sám sebe
				
				p2p_port = other_core.get('p2p_port', 0)
				if p2p_port:
					Append2File(f"P2P_ACCEPT: {v_bindHostname} {p2p_port}", config_path)
					print(f"  Added P2P: {current_core['path']} -> {other_core['path']} ({p2p_port})")
